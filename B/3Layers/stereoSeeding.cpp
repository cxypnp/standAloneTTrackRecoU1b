...
public:
    ...
    track(hit3 *_hitT1X, hit1 **_hitT1Set, hit3 **_hitT2Set, hit1 **_hitT3Set, bool _match)
    {
        hitT1X = _hitT1X;
        _hitT1X->used++;
        _hitT1X->relatedTracks.push_back(this);
        xMeasure[0] = hitT1X->x;
        for (Int_t j = 0; j < 4; j++)
        {
            if (j < 2)
        ...
        if (not trackMatch)
        {
            for (Int_t j = 0; j < 4; j++)
            {
                if (nTrueHits < 5)
                {
                    if ((hitT1X->motherTrack != NULL) && (*(hitT1X->motherTrack) == *this) > nTrueHits)
                    {
                        nTrueHits = (*(hitT1X->motherTrack) == *this);
                        motherTrack = hitT1X->motherTrack;
                        theTrueHit = 0;
                    }
                    if (j < 2)
                    {
    ...
    track(hit3 *_hitT1X, hit1 **_hitT1Set, hit3 **_hitT2Set, hit1 **_hitT3Set, int _nTrueHits)
    {
        if (_hitT1X != NULL)
            hitT1X = _hitT1X;
        for (Int_t j = 0; j < 4; j++)
    ...
    {
        int sum = 0;
        if ((this->hitT1X != NULL) && (anotherTrack.hitT1X != NULL))
        {
            sum += (this->hitT1X == anotherTrack.hitT1X);
        }
        for (Int_t j = 0; j < 4; j++)
        {
...
void stereoSeeding(TString settings = "p0", TString inputfilestring = "3LayerBig.root", TString outputplotsfolder = "3LayerU1Big/10HitDef/")
{
    ...
    float DY1T1[5] = {10, 20, 35, 40, 40};
    float DX1T1[5] = {2, 15, 35, 50, 60};

    int correctionT2[5] = {5, 10, 35, 40, 45};
    int nShareHits[5] = {8, 7, 6, 5, 4};
    int nGlobalShareHits = 3;
    ...
    // T1 x-u-v-x hits
    float hitT1X1[3] = {0, 0, 0};
    float hitT1up[2] = {0, 0};
    float hitT1down[2] = {0, 0};
    // Silicon Hits of T2
    ...
    if (rTracks)
    {
        ...
        rTracks->SetBranchAddress("t1x", &hitT1X1[0]);
        rTracks->SetBranchAddress("t1y", &hitT1X1[1]);
        rTracks->SetBranchAddress("t1z", &hitT1X1[2]);
        rTracks->SetBranchAddress("t1vu", &hitT1up[0]);
        rTracks->SetBranchAddress("t1vd", &hitT1down[0]);
        rTracks->SetBranchAddress("t1x2u", &hitT1up[1]);
        rTracks->SetBranchAddress("t1x2d", &hitT1down[1]);
    ...
    // T1 Figures
    TH1F *h1ErrT1X1XGood = new TH1F("h1ErrT1X1XGood", "h1ErrT1X1XGood", 160, 0, 160);
    h1ErrT1X1XGood->GetXaxis()->SetTitle("X_{pred}-X [mm]");
    TH1F *h1ErrT1X1XBad = new TH1F("h1ErrT1X1XBad", "h1ErrT1X1XBad", 160, 0, 160);
    h1ErrT1X1XBad->GetXaxis()->SetTitle("X_{pred}-X [mm]");
    ...
    TH1F *h1ErrT1X1YGood = new TH1F("h1ErrT1X1YGood", "h1ErrT1X1YGood", 160, 0, 80);
    h1ErrT1X1YGood->GetXaxis()->SetTitle("X_{pred}-X [mm]");
    TH1F *h1ErrT1X1YBad = new TH1F("h1ErrT1X1YBad", "h1ErrT1X1YBad", 160, 0, 80);
    h1ErrT1X1YBad->GetXaxis()->SetTitle("X_{pred}-X [mm]");
    ...
    // Container of hits
    SiliconLayer *T1LayerX = new SiliconLayer();
    SciFiLayer *T1Layers[2];
    ...
    unsigned int nReconstructable9Hit = 0;
    ...
    for (Int_t i = 0; i < nEntries; i++)
    {
        ...
        if (previousEventnumber != -1 and (previousEventnumber != eventNumber or i == nEntries - 1))
        {
            ...
            cout << "Event summary by layers: T1 " << T1LayerX->hits.size() << " " << T1LayersUp[0].hits.size() << " " << T1LayersUp[1].hits.size() << " " << T1LayersDown[0].hits.size() << " " << T1LayersDown[1].hits.size() << " T2: " << T2Layers[0].hits.size() << " " << T2Layers[1].hits.size() << " T3: " << T3LayersDown[0].hits.size() << " " << T3LayersDown[1].hits.size() << " " << T3LayersDown[2].hits.size() << " " << T3LayersDown[3].hits.size() << " " << endl;
            ...
            for (int iCase = iCases; iCase < nCases; iCase++)
            {
                for (vector<hit3 *>::iterator iT2x1 = T2Layers[0].hits.begin(); iT2x1 != T2Layers[0].hits.end(); iT2x1++)
                {
                    ...
                    // New track variables
                    // Prediction
                    float xPreT1[3] = {0.0, 0.0, 0.0};
                    float yPreT1[3] = {0.0, 0.0, 0.0};
                    float vPreT1 = 0.0;
...
// Tilt modification
                            ...
                            for (Int_t iT3x2 = 0; iT3x2 < T3Layers[3]->hits.size(); iT3x2++)
                            {
                                ...
                                for (Int_t iT3u1 = 0; iT3u1 < T3Layers[1]->hits.size(); iT3u1++)
                                {
                                    ...
                                    for (Int_t iT3v1 = 0; iT3v1 < T3Layers[2]->hits.size(); iT3v1++)
                                    {
                                        ...
                                        // Predict x at x1 and x2 of T1
                                        xPreT1[0] = fitResult[0];
                                        // xPreT1[1] = fitResult[1];
                                        // xPreT1[2] = fitResult[2];
                                        xPreT1[2] = fitResult[3];
                                        // Predict y at V and x1 of T1 TODO
                                        yPreT1[0] = trackT2[0]->y * 892.0 / 210.1 - trackT2[1]->y * 681.9 / 210.1;
                                        // v
                                        ...
                                        for (Int_t iT1x2 = 0; iT1x2 < T1Layers[1]->hits.size(); iT1x2++)
                                        {
                                            // cout << "iT1x2" << endl;
                                            trackT1[1] = T1Layers[1]->hits[iT1x2];
                                            if (trackT1[1]->flag)
                                                continue;
                                            sErrT1[3] = TMath::Abs(xPreT1[2] - trackT1[1]->s);
                                            ...
                                            for (Int_t iT1x1 = 0; iT1x1 < T1LayerX->hits.size(); iT1x1++)
                                            {
                                                // cout << "iT1x1" << endl;
                                                trackT1X = T1LayerX->hits[iT1x1];
                                                if (trackT1X->flag)
                                                    continue;
                                                sErrT1[0] = TMath::Abs(xPreT1[0] - trackT1X->x);
                                                sErrT1[1] = TMath::Abs(yPreT1[0] - trackT1X->y);

                                                if ((trackT2[0]->motherTrack != NULL) and (trackT2[0]->motherTrack->hitT1X != NULL) and (trackT2[0]->motherTrack->hitT1X == trackT1X))
                                                {
                                                    match[6] = true;
                                                }
                                                else
                                                ...
                                                {
                                                    h1ErrT1X1XGood->Fill(sErrT1[0]);
                                                    h1ErrT1X1YGood->Fill(sErrT1[1]);
                                                }
                                                else if (match[0] and match[1] and match[2] and match[3] and match[4] and match[5] and !match[6])
                                                {
                                                    h1ErrT1X1XBad->Fill(sErrT1[0]);
                                                    h1ErrT1X1YBad->Fill(sErrT1[1]);
                                                }

                                                if (sErrT1[0] > DX1T1[iCase])
                                                    continue;
                                                else if (sErrT1[1] > DY1T1[iCase])
                                                    continue;

                                                // Prediction of x on v of T1
                                                xPreT1[1] = trackT1[1]->s * 139.1 / 209.9 + trackT1X->x * 70.8 / 209.9;

                                                // Convert Prediction in y to u and v
                                                vPreT1 = xPreT1[1] * cos(5.0 / 180) - yPreT1[1] * sin(5.0 / 180);

                                                if (vUpOrDown(xPreT1[1], yPreT1[0]))
                                                {
                                                ...
                                                for (Int_t iT1v1 = 0; iT1v1 < T1Layers[0]->hits.size(); iT1v1++)
                                                {
                                                    // cout << "iT1v1" << endl;
                                                    trackT1[0] = T1Layers[0]->hits[iT1v1];
                                                    if (trackT1[0]->flag)
                                                        continue;
                                                    sErrT1[2] = TMath::Abs(vPreT1 - trackT1[0]->s);
                                                    ...
                                                    track *trackCandidate = new track(trackT1X, trackT1, trackT2, trackT3, match[0] and match[1] and match[2] and match[3] and match[4] and match[5] and match[6] and match[7]);
                    ...
                    if (!conf_alone)
                    {
                        ...
                        if (usedSum > 9)
                        {
                            ...
                            for (Int_t j = 0; j < 4; j++)
                            {
                                ...
                                if (j == 0 and (*_t)->hitT1X->used != 1)
                                {
                                    // Loop on every other related tracks.
                                    for (auto p : (*_t)->hitT1X->relatedTracks)
                                    {
                                ...
                                if (j < 2 and (*_t)->hitT1Set[j]->used != 1)
                                {
                                    // Loop on every other related tracks.
            ...
            for (_t = allTracks.begin(); _t != allTracks.end(); _t++)
            {
                ...
                if ((*_t)->trackAccept)
                {
                    ...
                    if ((*_t)->trackMatch)
                    {
                        ...
                        if ((*_t)->theTrueHit == 0)
                        {
                            if ((*_t)->hitT1X->motherTrack->particleMatched)
                            {
                                nClone++;
                                delete *_t;
                                continue;
                            ...
                            {
                                (*_t)->hitT1X->motherTrack->particleMatched = true;
                            }
                        }
                        else if ((*_t)->theTrueHit < 4)
                        {
                            if ((*_t)->hitT1Set[(*_t)->theTrueHit - 1]->motherTrack->particleMatched)
                            {
                                nClone++;
                                delete *_t;
                                continue;
                            ...
                            {
                                (*_t)->hitT1Set[(*_t)->theTrueHit - 1]->motherTrack->particleMatched = true;
                            }
                        }
            ...
            for (auto p : motherTracks)
            {
                if (!p->particleMatched)
                {
                    ...
                    {
                        if (j == 0 and !p->hitT1X)
                            h1_whereNotFound->Fill(0);
                        if (j < 2 and !p->hitT1Set[j])
                            h1_whereNotFound->Fill(j + 1);
        ...
        else
        {
            ...
            if (rTracks)
            {
                ...
                for (Int_t j = 0; j < 10; j++)
                {
                    if (j == 0)
                    {
                        if (not(hitT1X1[0] == 0 and hitT1X1[1] == 0 and hitT1X1[2] == 0))
                        {
                            _hitT1X = new hit3(hitT1X1[0], hitT1X1[1], hitT1X1[2]);
                            _hitT1X->hP = p;
                            _hitT1X->hEta = eta;
                            _hitT1X->eventID = eventNumber;
                            T1LayerX->hits.push_back(_hitT1X);
                            bT1X1Sum++;
                        }
                    }
                    if (j < 2)
                ...
                if ((bT1X1Sum + bT1X2Sum + bT2Sum + bT3XSum + bT3UVSum) == 0)
                {
                    continue;
                }
            }
            ...
            if (not((bT1X1Sum or bT1X2Sum == 2) and bT2Sum and bT3XSum and bT3UVSum and ((bT1X1Sum + bT1X2Sum + bT2Sum + bT3XSum + bT3UVSum) == 9)))
            // if (not((bT1X1Sum or (bT1X2Sum == 2)) and bT2Sum and bT3XSum and bT3UVSum and ((bT1X1Sum + bT1X2Sum + bT2Sum + bT3XSum + bT3UVSum) <= 9)))
            {
                continue;
            }
            ...
            // {
            //     //     cout << i << endl;
            //     // cout << (bT1X1Sum + bT1X2Sum + bT2Sum + bT3XSum + bT3UVSum ) << endl;
            //     cout << bT1X1Sum << " " << bT1X2Sum << " " << bT2Sum << " " << bT3XSum << " " << bT3UVSum << endl;
            //     cout << bT1X1Sum << endl;
            //     cout << i << endl;
            ...
            {
                nReconstructible++;
                if ((bT1X1Sum + bT1X2Sum + bT2Sum + bT3XSum + bT3UVSum) == 9)
                    nReconstructable9Hit++;
            ...
            track *_motherTrack = new track(_hitT1X, _hitT1Set, _hitT2Set, _hitT3Set, bT1X1Sum + bT1X2Sum + bT2Sum + bT3XSum + bT3UVSum);
            motherTracks.push_back(_motherTrack);
            if (_hitT1X != NULL)
            {
                _hitT1X->motherTrack = _motherTrack;
            }
            for (Int_t j = 0; j < 4; j++)
            {
                if (_hitT3Set[j] != NULL)
    ...
    logOut << "nReconstructable9Hit: " << nReconstructable9Hit << endl;
    ...
    h1ErrT1X1XGood->Draw();
    c1->SaveAs(outputplotsfolder + "/h1/" + "h1ErrT1X1XGood.png");
    h1ErrT1X1XBad->Draw();
    c1->SaveAs(outputplotsfolder + "/h1/" + "h1ErrT1X1XBad.png");

    h1ErrT1X1YGood->Draw();
    c1->SaveAs(outputplotsfolder + "/h1/" + "h1ErrT1X1YGood.png");
    h1ErrT1X1YBad->Draw();
    c1->SaveAs(outputplotsfolder + "/h1/" + "h1ErrT1X1YBad.png");
    ...
    make_inefficiency_window_plot(c1, h1ErrT1X1XGood, h1ErrT1X1XBad, outputplotsfolder);
    make_inefficiency_window_plot(c1, h1ErrT1X1YGood, h1ErrT1X1YBad, outputplotsfolder);
