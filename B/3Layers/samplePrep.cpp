...
std::shared_ptr<hit> recoverhit(float posz, std::vector<std::shared_ptr<hit>> inputs)
{
	...
	if (inputs.size() == 2)
	{
		...
		besthit1y = fy->Eval(posz);
		besthit1x = fx->Eval(posz);
		posz = posz + besthit1y / (1.0 / 0.003601 - fy->GetParameter(1));
		besthit1x = besthit1x + fx->GetParameter(1) * besthit1y / (1.0 / 0.003601 - fy->GetParameter(1));
		besthit1y = besthit1y * (1.0 / (1.0 - 0.003601 * fy->GetParameter(1)));
	}
	...
	if (inputs.size() == 1)
	{
		...
		posz = posz + besthit1y / (1.0 / 0.003601 - fy->GetParameter(1));
		besthit1x = besthit1x + fx->GetParameter(1) * besthit1y / (1.0 / 0.003601 - fy->GetParameter(1));
		besthit1y = besthit1y * (1.0 / (1 - 0.003601 * fy->GetParameter(1)));
	}
...
// void samplePrep(Int_t surface = 3, TString inputFile = "../../DataSets/original/MCtracks_U1b_small.root", TString outputFile = "3LayerSmall.root", bool blurIt = true)
void samplePrep(Int_t surface = 3, TString inputFile = "../../DataSets/original/MCtracks_U1b-5000ev.root", TString outputFile = "3LayerBig.root", bool blurIt = true)
{
	// For 3 layers // For 3 layers // For 3 layers // For 3 layers // For 3 layers
	...
	bool bT1HitX = false;
	bool bT1HitUp[2] = {false, false};
	bool bT1HitDown[2] = {false, false};
	...
	output_tree2->Branch("t1x", &hitT1X1[0], "t1x/F");
	output_tree2->Branch("t1y", &hitT1X1[1], "t1y/F");
	output_tree2->Branch("t1z", &hitT1X1[2], "t1z/F");
	output_tree2->Branch("t1vu", &hitT1up[0], "t1vu/F");
	output_tree2->Branch("t1vd", &hitT1down[0], "t1vd/F");
	output_tree2->Branch("t1x2u", &hitT1up[1], "t1x2u/F");
	output_tree2->Branch("t1x2d", &hitT1down[1], "t1x2d/F");
	...
	for (int i = 0; i < nentries; i++)
	{
		...
		for (int j = 29; j >= 0; j--)
		{
			...
			else if (_hit->z > 7950 && _hit->z < 7980)
			{
				...
				{
					hitT1up[0] = _hit->x * cos(5.0 / 180) - _hit->y * sin(5.0 / 180);
					bT1HitUp[0] = true;
				}
				else
				{
					hitT1down[0] = _hit->x * cos(5.0 / 180) - _hit->y * sin(5.0 / 180);
					bT1HitDown[0] = true;
				}
		...
		for (int k = 1; k < 7; k++)
		{
			...
			if (inputs.size() > 0)
			{
				...
				else
				{
					...
					if (k == 1 and IsInSiliconArea(_hit->x, _hit->y, surface))
					{
						hitT1X1[0] = _hit->x;
						hitT1X1[1] = _hit->y;
						hitT1X1[2] = _hit->z;
					...
					if ((k == 3 or k == 4) and IsInSiliconArea(_hit->x, _hit->y, surface))
					{
						hitT2[k - 3][0] = _hit->x;
						hitT2[k - 3][0] = _hit->y;
						hitT2[k - 3][0] = _hit->z;
		...
		if (blurIt)
		{
			if (bT1HitX)
			{
				if (IsInSiliconArea(hitT1X1[0], hitT1X1[1], surface))
				{
					hitT1X1[0] += rndgen.Gaus(0.0, 0.050 / sqrt(12));
					hitT1X1[1] += rndgen.Gaus(0.0, 0.150 / sqrt(12));
				}
				else
				{
					hitT1X1[0] += rndgen.Gaus(0.0, 0.070);
					hitT1X1[1] += rndgen.Gaus(0.0, 1.134);
				}
			}
			for (int j = 0; j < 4; j++)
			{
